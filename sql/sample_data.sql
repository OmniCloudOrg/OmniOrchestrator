...
INSERT INTO allocations (name, cpu, memory, uplink, downlink, disk, tier, price_per_hour)
VALUES
('free-tier', 0.5, 512, 1, 1, 1024, 'free', 0),
('starter-small', 1, 1024, 5, 5, 2048, 'starter', 0.012),
('starter-medium', 2, 2048, 10, 10, 4096, 'starter', 0.024),
('standard-small', 2, 4096, 25, 25, 10240, 'standard', 0.048),
('standard-medium', 4, 8192, 50, 50, 20480, 'standard', 0.096),
('standard-large', 8, 16384, 100, 100, 40960, 'standard', 0.192),
('performance-small', 16, 32768, 200, 200, 81920, 'performance', 0.384),
('performance-medium', 32, 65536, 400, 400, 163840, 'performance', 0.768),
('performance-large', 64, 131072, 800, 800, 327680, 'performance', 1.536),
('premium-small', 96, 196608, 1200, 1200, 491520, 'premium', 2.304),
('premium-medium', 128, 262144, 1600, 1600, 655360, 'premium', 3.072),
('premium-large', 192, 393216, 2400, 2400, 983040, 'premium', 4.608);
...
-- Org Admin has all except delete and billing
INSERT INTO permissions_role (permissions_id, role_id)
SELECT p.id, r.id
FROM permissions p, roles r
WHERE r.name = 'org_admin' 
AND p.action != 'delete'
AND p.name != 'org.billing';

-- Space Developer
INSERT INTO permissions_role (permissions_id, role_id)
SELECT p.id, r.id
FROM permissions p, roles r
WHERE r.name = 'space_developer' 
AND (p.resource_type = 'application' OR p.resource_type = 'service' OR p.resource_type = 'route' OR p.resource_type = 'config');

-- 5. Populate regions
INSERT INTO regions (name, display_name, provider, provider_region, location, coordinates, status, is_public, tier)
VALUES
...
-- 6. Populate users - Direct inserts without temp tables
-- Insert a few users with hardcoded values for testing
...
INSERT INTO user_pii (user_id, first_name, last_name, full_name, identity_verified)
VALUES
(@USER_COUNT + 1, 'System', 'Administrator', 'System Administrator', 1),
(@USER_COUNT + 2, 'Support', 'Team', 'Support Team', 1),
(@USER_COUNT + 3, 'Billing', 'Department', 'Billing Department', 1);
...
INSERT INTO role_user (user_id, role_id, scope_type)
VALUES
(@USER_COUNT + 1, (SELECT id FROM roles WHERE name = 'system_admin'), 'global');
...
-- Add admin quota
-- INSERT INTO quotas (name, org_id, memory_limit, cpu_limit, disk_limit, instance_limit, routes_limit, services_limit)
-- VALUES ('cloudplatform-quota', @ORG_COUNT + 1, 1048576, 512, 5242880, 1000, 1000, 500);
...
-- 9. Populate nodes
INSERT INTO nodes (region_id, name, provider_id, instance_type, status, cpu_total, cpu_available, cpu_reserved, memory_total, memory_available, memory_reserved, disk_total, disk_available, disk_reserved, network_in_capacity, network_out_capacity, kubernetes_version, labels, last_heartbeat, created_at)
VALUES
...
(10, 'node-u3v4w5x6', 'i-8901234567abcdefi', 'r5.8xlarge', 'active', 16, 4, 12, 131072, 31072, 100000, 1024000, 224000, 800000, 10000, 10000, '1.26.1', JSON_OBJECT('role', 'worker', 'zone', 'zone-a'), NOW() - INTERVAL 6 MINUTE, '2022-10-05 10:15:00'),
(10, 'node-y7z8a9b0', 'i-9012345678abcdefj', 'c5.4xlarge', 'active', 8, 3, 5, 32768, 12768, 20000, 512000, 212000, 300000, 10000, 10000, '1.26.1', JSON_OBJECT('role', 'worker', 'zone', 'zone-a'), NOW() - INTERVAL 9 MINUTE, '2022-10-10 14:30:00');
