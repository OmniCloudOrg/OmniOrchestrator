// backup/coordinator/network.rs
//
// Handles network configuration backup
...
pub async fn backup_network_config(
    backup: &Backup,
    ...
    match result {
        Ok(response) => {
            ...
            let dest_path = backup_dir.join("isos").join(format!("Network-Configuration-ISO-{}.iso", backup.id));
        ...
        Err(e) => {
            ...
            let job = BackupJobStatus {
                ...
                progress: 0.0,
                iso_path: None,
                error: Some(format!("Failed to backup network configuration: {}", e)),
                started_at: Utc::now(),
                completed_at: Some(Utc::now()),
                size_bytes: 0,
            };
