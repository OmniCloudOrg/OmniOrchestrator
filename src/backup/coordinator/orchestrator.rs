// backup/coordinator/orchestrator.rs
//
// Handles orchestrator component backup
...
/// Backup Orchestrator node
pub async fn backup_orchestrator(
    backup: &Backup,
    ...
    match result {
        Ok(response) => {
            ...
            let dest_path = backup_dir.join("isos").join(format!("Orchestrator-State-ISO-{}-{}.iso", node.name.replace(" ", "-"), backup.id));
        ...
        Err(e) => {
            ...
            let job = BackupJobStatus {
                ...
                progress: 0.0,
                iso_path: None,
                error: Some(format!("Failed to backup orchestrator: {}", e)),
                started_at: Utc::now(),
                completed_at: Some(Utc::now()),
                size_bytes: 0,
            };
