// backup/coordinator/system_core.rs
//
// Handles system core component backup
...
/// Backup System Core from master node
pub async fn backup_system_core(
    backup: &Backup,
    ...
    match result {
        Ok(response) => {
            ...
            let dest_path = backup_dir.join("isos").join(format!("System-Core-ISO-{}.iso", backup.id));
        ...
        Err(e) => {
            ...
            let job = BackupJobStatus {
                ...
                progress: 0.0,
                iso_path: None,
                error: Some(format!("Failed to backup system core: {}", e)),
                started_at: Utc::now(),
                completed_at: Some(Utc::now()),
                size_bytes: 0,
            };
