...
use crate::db::v1::tables::{Provider, Instance};
...
pub async fn get_provider_instances(
    ...
    let instances: Vec<Instance> = match crate::db::v1::queries::provider::get_provider_instances(&pool, provider_id, page, per_page).await {
        Ok(instances) => {
    ...
    let total_count = match crate::db::v1::queries::provider::get_provider_instance_count(&pool, provider_id).await {
        Ok(count) => {
